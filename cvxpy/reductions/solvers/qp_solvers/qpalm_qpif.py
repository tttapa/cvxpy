...
class QPALM(QpSolver):
    ...
    def invert(self, solution, inverse_data):
        ...
        if status in s.SOLUTION_PRESENT:
            ...
            primal_vars = {
                QPALM.VAR_ID:
                intf.DEFAULT_INTF.const_to_matrix(solution.solution.x)
            }
            dual_vars = {
                QPALM.DUAL_VAR_ID:
                intf.DEFAULT_INTF.const_to_matrix(solution.solution.y)
            }
    ...
    def solve_via_data(self, data, warm_start: bool, verbose: bool, solver_opts,
        ...
        if warm_start and self.name() in solver_cache:
            ...
            sp_neq = lambda a, b: a.data.shape != b.data.shape or any(a.data != b.data)
